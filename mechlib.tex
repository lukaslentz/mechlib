...
\tikzset{
	...
	pics/roller/.default = {0.3cm,0.2cm},
}

% ------------------ Pined (Gelenklager) ------------------
% Pic: pined
% Parameter: #1 Dimension, #2 Radius
\tikzset{
	pics/pined/.style args={#1,#2}{
		code={
			\def\mlw{#1}
			\def\mlh{#1}
			\def\mlr{#2}
...
% Parameter: #1 Dimension, #2 Radius
\tikzset{
	pics/fixed/.style args={#1,#2}{
		code={
			\def\mlw{#1}
			\def\mlh{#1}
			\draw[] (-0.5*\mlw,0) -- (0.5*\mlw,0);
			\fill[
			pattern={Lines[angle=45, distance={3pt/sqrt(2)}]},
			pattern color=black
			] (-0.5*\mlw,0) rectangle (0.5*\mlw,-\mlh);
		}
	},
	pics/fixed/.default = {.2cm,.4mm},
}
