% =========================================================
% TikZ / Mathe / Farben – Stylesammlung
% (06.02.2026/Lentz: Formatierung aufgeräumt, kommentiert; Inhalt unverändert)
% (11.02.2026/Reinsch:
%		- Balkendefinition angepasst, sodass vertikale Balken, schräge Balken und kombinierte Balken möglich sind
%		- Neue Funktion idim, intelligente Bemaßung. Unterstützt horizontal, vertikal und schräge Bemaßungen
%		- Funktion f überarbeitet, mehr Parameter und Anpassbarkeit
%		- Funktion dl_uniform und dl_linear angepasst, sodass Name vergeben werden kann und Farben konsistent gemacht
%		- Neue Funktionen für Fachwerke (\makeJoints und draw rod)
% )
% =========================================================

% --- Mathe / Einheiten / Farben ---
\usepackage{amsmath}
\usepackage{siunitx}
\usepackage{xcolor}

% --- TikZ Libraries ---
\usetikzlibrary{
	arrows.meta,
	calc,
	patterns,
	patterns.meta,
	intersections,
	angles,
	quotes,
	shapes.geometric,
	shadings,
	decorations.pathmorphing
}

% --- Farbschema ---
\definecolor{ucbblue}{RGB}{6,13,141}
\definecolor{ucbgreen}{RGB}{67,176,42}
\definecolor{mmdblue}{rgb}{0.2,0.2,0.7}
\definecolor{tured}{rgb}{0.7,0.1,0.1}

% --- Standardfont in TikZ-Bildern ---
\tikzset{
	every picture/.style={font=\fontsize{10}{10}\selectfont}
}

% --- Layer für Fachwerke ---
\pgfdeclarelayer{background}
\pgfsetlayers{background,main}

% --- Standard-Schrift: serifenlos (Text im Dokument) ---
\renewcommand{\familydefault}{\sfdefault}

\def\hatchangle{45}

% =========================================================
% Mechanical Elements
% =========================================================

% ------------------ Farbstile ------------------
% Körper / Bauteile
\tikzset{
	body/.style={
		black,
		fill=gray!40
	},
}


% Lager / Auflager
\tikzset{
	support/.style={
		black,
		fill=gray!30
	},
}

% Lasten (distributed loads)
\tikzset{
	dl/.style={
		red
	},
	dl_fill/.style={
		opacity=0.2,
		red!50
	},
}

% ------------------ Box ------------------
% Pic: box
% Parameter: #1 Breite, #2 Höhe
\tikzset{
	pics/box/.style args={#1,#2}{%
		code={
			\draw[body] (-0.5*#1,-0.5*#2) rectangle (0.5*#1,0.5*#2);
		}
	},
	pics/box/.default={2cm,1cm},
}

% ------------------ Ball ------------------
% Pic: ball
% Parameter: #1 Radius, #2 Farbe
\tikzset{
	pics/ball/.style args={#1,#2}{%
		code={
			\shade[ball color=#2, shading angle=0, draw=none] (0,0) circle [radius=#1];
		}
	},
	pics/ball/.default={0.5,red},
}

% ------------------ Beam ------------------
% Pic: beam
% Parameter: #1 Punkt A, #2 Punkt B, #3 Höhe (Offset)
\tikzset{
	pics/beam/.style args={#1,#2,#3}{
		code={
			% 1. Geometrie berechnen
			\path let
			\p1 = (#1),
			\p2 = (#2),
			\n1 = {veclen(\x2-\x1,\y2-\y1)},
			\n2 = {(\y2-\y1)/\n1 * #3/2},
			\n3 = {-(\x2-\x1)/\n1 * #3/2}
			in
			% 2. Rechteck-Ecken
			coordinate (-A) at (\x1+\n2,\y1+\n3)
			coordinate (-B) at (\x2+\n2,\y2+\n3)
			coordinate (-C) at (\x2-\n2,\y2-\n3)
			coordinate (-D) at (\x1-\n2,\y1-\n3)
			
			% 3. Koppel-Anker
			coordinate (-start-left)  at (-D)
			coordinate (-start-right) at (-A)
			coordinate (-end-left)    at (-C)
			coordinate (-end-right)   at (-B);
			
			% 4. Balken zeichnen
			\draw[body] (-A) -- (-B) -- (-C) -- (-D) -- cycle;
		}
	}
}


% =========================================================
% Supports
% =========================================================

% ------------------ Roller (Loslager) ------------------
% Pic: roller
% Parameter: #1 Breite, #2 Höhe, #3 Schraffurwinkel
\tikzset{
	pics/roller/.style args={#1,#2,#3}{
		code={
			\def\mlw{0.2cm} \def\mlh{0.2cm} \def\mlr{0.06cm}
			% Lagerdreieck
			\draw[support] (0,0) -- (-0.5*\mlw, \mlh-1pt) --+ (\mlw,0) -- cycle;
			\draw[support] (0,0) circle (\mlr);
			
			% Schraffierte Bodenplatte
			\fill[
			pattern={Lines[angle=#3, distance={1pt}]},
			pattern color=black
			] (-0.5*\mlw,\mlh) rectangle (0.5*\mlw,1.4*\mlh);
			
			\draw (-0.5*\mlw, \mlh) -- (0.5*\mlw, \mlh);
		}
	},
	pics/roller/.default = {0.3cm,0.2cm,45},
}

% ------------------ Pined (Gelenklager) ------------------
% Pic: pined
% Parameter: #1, #2 (aktuell nicht genutzt), #3 Schraffurwinkel
\tikzset{
	pics/pined/.style args={#1,#2,#3}{
		code={
			\def\mlw{0.2cm}
			\def\mlh{0.2cm}
			\def\mlr{0.06cm}
			\draw[support] (0,0) -- (-0.5*\mlw, \mlh) --+ (\mlw,0) -- cycle;
			\draw[support] (0,0) circle (\mlr);
			
			\fill[
			pattern={Lines[angle=#3, distance={1pt}]},
			pattern color=black
			] (-0.5*\mlw,\mlh) rectangle (0.5*\mlw,1.4*\mlh);
		}
	},
	pics/pined/.default = {1cm,1cm,45},
}

% ------------------ Fixed Support (Einspannung) ------------------
% Pic: fixed
% Parameter: #1 Breite, #2 Höhe, #3 Schraffurwinkel
\tikzset{
	pics/fixed/.style args={#1,#2,#3}{
		code={
			\def\mlw{#1}
			\def\mlh{#2}
			\draw[] (-0.5*\mlw,0) -- (0.5*\mlw,0);
			\fill[
			pattern={Lines[angle=#3, distance={3pt/sqrt(2)}]},
			pattern color=black
			] (-0.5*\mlw,0) rectangle (0.5*\mlw,-\mlh);
		}
	},
	pics/fixed/.default = {1cm,0.1cm,45},
}


% =========================================================
% Loads
% =========================================================

% ------------------ Point Force ------------------
% Pic: f
% Parameter: #1 = Startpunkt, #2 = Richtung (0 = Spitze am Punkt, 1 = Spitze vom Punkt weg),
%            #3 = Länge des Pfeils, #4 = Winkel in Grad (0 = rechts), #5 = Beschriftung
\tikzset{
	pics/f/.style args={#1,#2,#3,#4,#5}{%
		code={%
			% Parameter
			\coordinate (-start) at (#1);
			\pgfmathsetmacro{\reverse}{#2}   % 0 oder 1
			\pgfmathsetlengthmacro{\len}{#3}
			\pgfmathsetmacro{\angle}{#4}
			
			% Pfeil je nach Richtung
			\ifnum\reverse=0
			% Spitze am Punkt (wie bisher)
			\coordinate (-tip) at (-start);
			\path (-tip) -- ++({-cos(\angle)*\len},{-sin(\angle)*\len}) coordinate (-tail);
			\draw[-latex,red,line width=1pt] (-tail) -- (-tip);
			
			% Beschriftung bei Tail (etwas außerhalb)
			\pgfmathsetmacro{\offset}{0.3*\len}
			\coordinate (-label) at ($(-tail)!-0.3!(-tip)$);
			\else
			% Spitze vom Punkt weg
			\coordinate (-tip) at ($(-start) + ({cos(\angle)*\len},{sin(\angle)*\len})$);
			\coordinate (-tail) at (-start);
			\draw[-latex,red,line width=1pt] (-tail) -- (-tip);
			
			% Beschriftung vor der Spitze (leicht außerhalb)
			\pgfmathsetmacro{\offset}{0.3*\len}
			\coordinate (-label) at ($(-tip)!-0.3!(-tail)$);
			\fi
			
			\node[red,anchor=center] at (-label) {#5};
		}
	}
}




% ------------------ Uniformly Distributed Load ------------------
% Pic: dl_uniform
% Parameter: #1 Punkt A, #2 Punkt B, #3 Offset in y, #4 Höhe der Lastdarstellung, #5 Beschriftung
\tikzset{
	pics/dl_uniform/.style args={#1,#2,#3,#4,#5}{
		code={
			\path let \p1=(#1), \p2=(#2) in
			coordinate (-A) at (\x1, \y1 + #3)
			coordinate (-B) at (\x2, \y2 + #3);
			
			\fill[dl_fill] (-A) rectangle ($(-B) + (0,#4)$);
			
			\draw[dl]
			($(-A) + (0,.5*#4)$) -- ($(-A) + (0,#4)$) -- ($(-B) + (0,#4)$) -- ($(-B) + (0,.5*#4)$);
			
			\foreach \dx in {0,0.2,...,1}{
				\draw[-latex,dl] ($(-A)!\dx!(-B) + (0,#4)$) -- +(0,-#4);
			}
			
			\node[anchor=south,dl] at ($(-B) + (0,#4)$) {#5};
		}
	},
	pics/dl_uniform/.default = {(0,0),(1,0),0.3cm,0.8cm,$q_0$},
}

% ------------------ Linearly Distributed Load ------------------
% Pic: dl_linear
% Parameter: #1 Punkt A, #2 Punkt B, #3 Offset in y, #4 Maximalhöhe, #5 Beschriftung
\tikzset{
	pics/dl_linear/.style args={#1,#2,#3,#4,#5}{
		code={
			\path let \p1=(#1), \p2=(#2) in
			coordinate (-A) at (\x1, \y1 + #3)
			coordinate (-B) at (\x2, \y2 + #3);
			
			\fill[dl_fill] (-A) -- ($(-B) + (0,#4)$) -- (-B) -- cycle;
			
			\draw[dl] (-A) -- ($(-B) + (0,#4)$) -- ($(-B) + (0,.5*#4)$);
			
			\foreach \dx in {0.2,0.4,...,1}{
				\draw[-latex,dl] ($(-A)!\dx!(-B) + (0,\dx*#4)$) -- +(0,-\dx*#4);
			}
			
			\node[anchor=south,dl] at ($(-B) + (0,#4)$) {#5};
		}
	},
	pics/dl_linear/.default = {(0,0),(1,0),0.3cm,0.8cm,$q_0$},
}


% =========================================================
% Dimensions
% =========================================================

% ------------------ Arc Dimension ------------------
% Pic: adim
% Parameter: #1 Startwinkel, #2 Endwinkel, #3 Text, #4 Radius
\tikzset{
	pics/adim/.style args={#1,#2,#3,#4}{%
		code={
			\def\mlr{#4}
			\draw[] (#1:\mlr) arc (#1:#2:\mlr);
			\node[] at (.5*#1 + .5*#2 :.7*\mlr){#3};
		}
	},
	pics/adim/.default={0,45,{$\alpha$},.7cm},
}

% ------------------ Horizontal Dimension ------------------
% Pic: hdim
% Parameter: #1 Punkt A, #2 Punkt B, #3 Text, #4 Offset in y
\tikzset{
	pics/hdim/.style args={#1,#2,#3,#4}{%
		code={
			\path let \p1=(#1), \p2=(#2) in
			coordinate (-A) at (\x1, \y1 + #4)
			coordinate (-B) at (\x2, \y2 + #4);
			
			\draw[latex-latex, line width=0.5pt] (-A) -- (-B)
			node[pos=0.5, below]{#3};
		}
	},
	pics/hdim/.default={(0,0),(1,0),{$l$},0.3cm},
}

% ------------------ Vertical Dimension ------------------
% Pic: vdim
% Parameter: #1 Punkt A, #2 Punkt B, #3 Text, #4 Offset in x
\tikzset{
	pics/vdim/.style args={#1,#2,#3,#4}{%
		code={
			\path let \p1=(#1), \p2=(#2) in
			coordinate (-A) at (\x1 + #4, \y1)
			coordinate (-B) at (\x2 + #4, \y2);
			
			\draw[latex-latex, line width=0.5pt] (-A) -- (-B)
			node[pos=0.5, left]{#3};
		}
	},
	pics/vdim/.default={(0,0),(0,1),{$h$},0.3cm},
}

% ------------------ Intelligent Dimension ------------------
% Pic: idim
% Parameter: #1 Punkt A, #2 Punkt B, #3 Text, #4 Offset in x/y
\tikzset{
	pics/idim/.style args={#1,#2,#3,#4}{%
		code={
			% Punkte
			\path (#1); \pgfgetlastxy{\xA}{\yA};
			\path (#2); \pgfgetlastxy{\xB}{\yB};
			
			% Differenzen
			\pgfmathsetmacro{\dx}{\xB-\xA}
			\pgfmathsetmacro{\dy}{\yB-\yA}
			
			% Horizontal oder vertikal erkennen
			\pgfmathsetmacro{\ishorizontal}{ifthenelse(abs(\dx) >= abs(\dy), 1, 0)}
			
			% Offset
			\pgfmathsetmacro{\ox}{\ishorizontal==1 ? 0 : #4}
			\pgfmathsetmacro{\oy}{\ishorizontal==1 ? #4 : 0}
			
			% Anchor für Beschriftung bestimmen
			\pgfmathsetmacro{\anchorcode}{%
				\ishorizontal==1 ? (#4>=0 ? "south" : "north") : (#4>=0 ? "west" : "east")%
			}
			\edef\myanchor{\anchorcode}
			
			% Zeichnen
			\draw[latex-latex,line width=0.5pt]
			(\xA+\ox,\yA+\oy) -- (\xB+\ox,\yB+\oy)
			node[midway, anchor=\myanchor]{#3};
		}
	},
	pics/idim/.default={(0,0),(1,0),{$l$},0.3cm}
}



% =========================================================
% Coordinate Systems
% =========================================================

% ------------------ 1-Koordinatensystem ------------------
% Pic: x_cos
% Parameter: #1 Länge, #2 Name, #3 Color
\tikzset{
	pics/x_cos/.style args={#1,#2,#3}{
		code={
			\draw[latex-,#3](0,0)--(#1,0);
			\coordinate (-tag) at ($(0,0)!.2!(#1,0)$);
			\draw[#3](#1,-0.5*#2) -- (#1,0.5*#2);
			\fill[
			pattern={Lines[angle=\hatchangle, distance={1pt}]},
			pattern color=#3
			] (#1,-0.5*#2) rectangle (#1+0.2*#2,0.5*#2);
		}
	},
	pics/x_cos/.default = {0.8cm,0.4cm,ucbblue},
}

% ------------------ x/y Koordinatensystem ------------------
% Pic: xy_cos
% Parameter: #1 x-Länge, #2 y-Länge
\tikzset{
	pics/xy_cos/.style args={#1,#2}{
		code={
			\draw[latex-latex]
			(#1,0)--(0,0)node[anchor=north,pos=0]{$x$}
			--(0,#2)node[anchor=east,pos=1]{$y$};
		}
	},
	pics/xy_cos/.default = {0.8cm,0.8cm},
}

% ------------------ x/z Koordinatensystem ------------------
% (Hinweis: Original verwendet (0,-\r); \r ist hier nicht definiert – unverändert gelassen.)
\tikzset{
	cosys/.pic={
		\def\mlr{1cm}
		\draw[latex-latex]
		(0,-\r) -- (0,0) node[pos=0,anchor=north] {$z$}
		-- (\mlr,0) node[pos=1,anchor=west]  {$x$};
		
		\draw[-latex, shorten >=-2pt]
		(-80:0.8*\mlr) arc (-80:-20:0.8*\mlr)
		node[pos=0.5,anchor=north west]{+};
	}
}


% =========================================================
% Reaction Forces
% =========================================================

% ------------------ Reaktionsgrößen an Einspannung ------------------
% Pic: sf_fixed
% Parameter: #1 Pfeillänge (Skalierung), #2 Höhe (Referenz)
\tikzset{
	pics/sf_fixed/.style args={#1,#2}{
		code={
			\def\mlf{#1}
			\def\mlh{#2}
			
			\draw[line width=0.5pt,-latex,red]
			($(0,0.5*\mlh)+(-0.8*\mlf,0)$) -- +(0.8*\mlf,0)
			node[pos=0,anchor=east,xshift=.1cm] {$A_x$};
			
			\draw[line width=0.5pt,-latex,red]
			($(0,0.5*\mlh)+(-1pt,0.4*\mlf)$) -- +(0,-\mlf)
			node[pos=0,anchor=south,yshift=-.1cm,xshift=-1mm] {$A_z$};
			
			\draw[line width=0.5pt,-latex,red,shorten >=-2pt]
			($(0,0.5*\mlh)+(120:0.4*\mlf)$) arc (120:230:0.4*\mlf)
			node[pos=1,anchor=east,yshift=-.1cm,xshift=1mm] {$M_A$};
		}
	},
	pics/sf_fixed/.default = {0.7cm,0.3cm},
}


% =========================================================
% Internal Forces
% =========================================================

% ------------------ Schnittgrößen positiv ------------------
% Pic: sl_pos
% Parameter: #1 Pfeillänge, #2 Höhe (Referenz)
\tikzset{
	pics/sl_pos/.style args={#1,#2}{
		code={
			\def\mlf{#1}
			\def\mlh{#2}
			
			\draw[line width=0.5pt,-latex,red] (0,0.5*\mlh) -- +(0.8*\mlf,0)
			node[pos=1,anchor=west,xshift=-.1cm] {$N$};
			
			\draw[line width=0.5pt,-latex,red]
			($(0,0.5*\mlh) + (1pt,0.4*\mlf)$) -- +(0,-\mlf)
			node[pos=0,anchor=south,yshift=-.1cm,xshift=.1cm] {$Q$};
			
			\draw[line width=0.5pt,-latex,red,shorten >=-2pt]
			($(0,0.5*\mlh) + (-60:0.4*\mlf)$) arc(-60:50:0.4*\mlf)
			node[pos=1,anchor=west,yshift=.1cm,xshift=.05cm] {$M$};
		}
	},
	pics/sl_pos/.default = {0.7cm,0.3cm},
}

% ------------------ Schnittgrößen negativ ------------------
% Pic: sl_neg
% Parameter: #1 Pfeillänge, #2 Höhe (Referenz)
\tikzset{
	pics/sl_neg/.style args={#1,#2}{
		code={
			\def\mlf{#1}
			\def\mlh{#2}
			
			\draw[line width=0.5pt,-latex,red] (0,0.5*\mlh) -- +(-0.8*\mlf,0)
			node[pos=1,anchor=east,xshift=.1cm] {$N$};
			
			\draw[line width=0.5pt,-latex,red]
			($(0,0.5*\mlh)+(-1pt,-0.4*\mlf)$) -- +(0,\mlf)
			node[pos=1,anchor=south,yshift=-.1cm,xshift=-.1cm] {$Q$};
			
			\draw[line width=0.5pt,-latex,red,shorten >=-2pt]
			($(0,0.5*\mlh)+(240:0.4*\mlf)$) arc (240:130:0.4*\mlf)
			node[pos=1,anchor=east,yshift=.1cm,xshift=-.05cm] {$M$};
		}
	},
	pics/sl_neg/.default = {0.7cm,0.3cm},
}

% =========================================================
% Trusses (Fachwerke)
% =========================================================

% ------------------ Knotendarstellung für Korrdinatenliste machen ------------------
% Usage: \makeJoints{K1, K2, K.., Kn}
\newcommand{\makeJoints}[1]{%
	\foreach \k in {#1}{%
		\draw[fill=gray!30, draw=black, line width=0.2mm] (\k) circle (0.6mm);
	}%
}

% ------------------ Balken anlegen ------------------
% Pic: rod
% Parameter: #1 Knoten 1, #2 Knoten 2, #3 Balkenname, #4 Anker, #5 xshift
\tikzset{
	pics/rod/.style args={#1,#2,#3,#4,#5}{
		code={
			\begin{pgfonlayer}{background}
				\draw[draw=gray!80, line width=2.5pt] (#1) -- (#2);
			\end{pgfonlayer}
			
			% Mittelpunkt
			\path (#1) -- (#2) coordinate[pos=0.5] (midpoint);
			
			
			% Node mit xshift
			\node[color=black, anchor=#4, shift={(#4:#5)}] at (midpoint) {#3};
		}
	}
}

% =========================================================
% Oscillators (Schwinger)
% =========================================================

% ------------------ Pendel ------------------
% Pic: Pendel
% Parameter: #1 Länge, #2 Winkel,#3 Radius der Masse, #4 Name der Masse, #5 Name der Länge
\tikzset{
	pics/pendel/.style args={#1,#2,#3,#4,#5}{
		code={
			
			\pgfmathsetlengthmacro{\ml}{#1 cm}
			\pgfmathsetmacro{\malph}{#2}
			\pgfmathsetlengthmacro{\mr}{#3 cm}
			
		
			\draw[gray!10](0,0)--(0,-\ml);
			\draw[](0,0)--(\malph:\ml);
			\draw[body] (\malph:\ml) circle (\mr);
			\node at (\malph:\ml) {#4};
			\pic{pined={1,1,45}};
			
%			\pic{adim={-90,\malph,$\alpha$,0.8}};
			
		}
	}
}

% ------------------ Bewegtes Pendel ------------------
% Pic: bewegtespendel
% Parameter: #1 Höhe, #2 Breite,#3 Name der Masse, #4 Federsteifigkeit, #5 Dämpfung, #6 Länge, #7 Winkel, #8 Radius der Masse
\tikzset{
	pics/bewegtespendel/.style args={#1,#2,#3,#4,#5,#6,#7,#8}{
		code={
			\pgfmathsetmacro{\mh}{0.8*#1}
			\pgfmathsetmacro{\mb}{0.5*#2}
			\pgfmathsetmacro{\mr}{0.1*#1}
			\pgfmathsetmacro{\mf}{2*\mr+0.25*\mh}
			\pgfmathsetmacro{\md}{2*\mr+0.75*\mh}
			\pgfmathsetlengthmacro{\ml}{#6 cm}
			\pgfmathsetmacro{\malph}{#7}
			\pgfmathsetlengthmacro{\mpr}{#8 cm}
			
			\pic at (\mb,0){wagen={\mh,\mb,\mr,$$}};
			\pic {feder={0,\mb,\mf,$c$}};
			\pic {daempfer={0,\mb,\md,$d$}};
			
			\fill[black] (1.5*\mb,2*\mr+.5*\mh) circle (1pt);
			\draw[] (1.5*\mb,2*\mr+.5*\mh) --+ (\malph:\ml);
			\draw[body] ($ (1.5*\mb,2*\mr+.5*\mh) + (\malph:\ml)$) circle (\mpr);
			
			% Wand
			\draw[gray](0,0) --++ (0,\mh+2*\mr);
			\fill[pattern={Lines[angle=45, distance={1pt}]},pattern color=black](0,0) rectangle (-3pt,\mh+2*\mr);
			
		}
	}
}

% ------------------ Wagen ------------------
% Pic: Wagen
% Parameter: #1 Höhe, #2 Breite, #3 Radius Räder, #4 Name der Masse
\tikzset{
	pics/wagen/.style args={#1,#2,#3,#4}{
		code={
			
			\pgfmathsetlengthmacro{\mh}{#1 cm}
			\pgfmathsetlengthmacro{\mb}{#2 cm}
			\pgfmathsetlengthmacro{\mr}{#3 cm}
			
			% Wagen
			\draw[body] (.2*\mb,\mr) circle (\mr);
			\draw[body] (.8*\mb,\mr) circle (\mr);
			\draw[body] (0,2*\mr) rectangle (\mb,2*\mr+\mh);
			\node[]at(.5*\mb,2*\mr+.5*\mh){#4};
			
			% Boden
			\draw[](0,0) --++ (\mb,0);
			\fill[pattern={Lines[angle=45, distance={1pt}]},pattern color=black](0,0) rectangle (\mb,-3pt);
			
		}
	}
}

% ------------------ Feder ------------------			
% Feder
% Pic: Feder
% Parameter: #1 Startx, #2 Endx, #3 Höhe, #4 Name
\tikzset{
	pics/feder/.style args={#1,#2,#3,#4}{
		code={

			\pgfmathsetlengthmacro{\mxs}{#1 cm}
			\pgfmathsetlengthmacro{\mxe}{#2 cm}
			\pgfmathsetlengthmacro{\my}{#3 cm}
			\pgfmathsetlengthmacro{\ml}{\mxe-\mxs}
			
			\draw[] 
			(\mxs,\my)--++(0.2*\ml,0)
			decorate[decoration={zigzag, segment length=.15*\ml, amplitude=.1*\ml}]{--++(0.6*\ml,0)node[pos=0.5,anchor=north,yshift=-.1*\ml]{#4}} 
			--++(0.2*\ml,0);
			
		}
	}
}

% ------------------ Dämpfer ------------------			
% Dämpfer
% Pic: Dämpfer
% Parameter: #1 Startx, #2 Endx, #3 Höhe, #4 Name
\tikzset{
	pics/daempfer/.style args={#1,#2,#3,#4}{
		code={
			
			\pgfmathsetlengthmacro{\mxs}{#1 cm}
			\pgfmathsetlengthmacro{\mxe}{#2 cm}
			\pgfmathsetlengthmacro{\my}{#3 cm}
			\pgfmathsetlengthmacro{\ml}{\mxe-\mxs}
			
			\draw[] (\mxs,\my)--++(0.3*\ml,0);
			\draw[] (0.7*\ml,\my-.1*\ml)--++(-.4*\ml,0)--++(0,.2*\ml)--++(0.4*\ml,0)node[pos=0.5,anchor=south]{#4};
			\draw[] (\ml,\my)--++(-0.4*\ml,0);
			
		}
	}
}

% ------------------ Einmassenschwinger ------------------
% Pic: einmassenschwinger
% Parameter: #1 Höhe, #2 Breite,#3 Name der Masse, #4 Federsteifigkeit, #5 Dämpfung
\tikzset{
	pics/einmassenschwinger/.style args={#1,#2,#3,#4,#5}{
		code={
			\pgfmathsetmacro{\mh}{0.8*#1}
			\pgfmathsetmacro{\mb}{0.5*#2}
			\pgfmathsetmacro{\mr}{0.1*#1}
			\pgfmathsetmacro{\mf}{2*\mr+0.25*\mh}
			\pgfmathsetmacro{\md}{2*\mr+0.75*\mh}
	
			\pic at (\mb,0){wagen={\mh,\mb,\mr,$m$}};
			\pic {feder={0,\mb,\mf,$c$}};
			\pic {daempfer={0,\mb,\md,$d$}};
			
			% Wand
			\draw[](0,0) --++ (0,\mh+2*\mr);
			\fill[pattern={Lines[angle=45, distance={1pt}]},pattern color=black](0,0) rectangle (-3pt,\mh+2*\mr);

		}
	}
}

% ------------------ Zweimassenschwinger ------------------
% Pic: einmassenschwinger
% Parameter: #1 Höhe, #2 Breite,#3 Name der Masse, #4 Federsteifigkeit, #5 Dämpfung
\tikzset{
	pics/zweimassenschwinger/.style args={#1,#2,#3,#4,#5}{
		code={
			\pgfmathsetmacro{\mh}{0.8*#1}
			\pgfmathsetmacro{\mb}{0.5*#2}
			\pgfmathsetmacro{\mr}{0.1*#1}
			\pgfmathsetmacro{\mf}{2*\mr+0.25*\mh}
			\pgfmathsetmacro{\md}{2*\mr+0.75*\mh}
			
			% Wand
			\draw[](0,0) --++ (0,\mh+2*\mr);
			\fill[pattern={Lines[angle=45, distance={1pt}]},pattern color=black](0,0) rectangle (-3pt,\mh+2*\mr);
			
			% Schwinger
			\pic at (\mb,0){wagen={\mh,\mb,\mr,$m_1$}};
			\pic {feder={0,\mb,\mf,$c_1$}};
			\pic {daempfer={0,\mb,\md,$d_1$}};
			
			\begin{scope}[xshift=2*\mb cm]
				% Schwinger
				\pic at (\mb,0){wagen={\mh,\mb,\mr,$m_2$}};
				\pic {feder={0,\mb,\mf,$c_2$}};
				\pic {daempfer={0,\mb,\md,$d_2$}};
			\end{scope}
			
			\begin{scope}[xshift=4*\mb cm]
				\pic {feder={0,\mb,\mf,$c_3$}};
				\pic {daempfer={0,\mb,\md,$d_3$}};
			\end{scope}
			
			\begin{scope}[xshift=5*\mb cm]
				% Wand
				\draw[](0,0) --++ (0,\mh+2*\mr);
				\fill[pattern={Lines[angle=-45, distance={1pt}]},pattern color=black](0,0) rectangle (3pt,\mh+2*\mr);
			\end{scope}
		
		}
	}
}

% ------------------ n-massenschwinger ------------------
% Pic: einmassenschwinger
% Parameter: #1 Höhe, #2 Breite,#3 Name der Masse, #4 Federsteifigkeit, #5 Dämpfung
\tikzset{
	pics/nmassenschwinger/.style args={#1,#2,#3,#4,#5}{
		code={
			\pgfmathsetmacro{\mh}{0.8*#1}
			\pgfmathsetmacro{\mb}{0.5*#2}
			\pgfmathsetmacro{\mr}{0.1*#1}
			\pgfmathsetmacro{\mf}{2*\mr+0.25*\mh}
			\pgfmathsetmacro{\md}{2*\mr+0.75*\mh}
			
			% Wand
			\draw[](0,0) --++ (0,\mh+2*\mr);
			\fill[pattern={Lines[angle=45, distance={1pt}]},pattern color=black](0,0) rectangle (-3pt,\mh+2*\mr);
			
			% Schwinger
			\pic at (\mb,0){wagen={\mh,\mb,\mr,$m_1$}};
			\pic {feder={0,\mb,\mf,$c_1$}};
			\pic {daempfer={0,\mb,\md,$d_1$}};
			
			\begin{scope}[xshift=2*\mb cm]
				% Schwinger
				\pic {feder={0,\mb,\mf,$c_2$}};
				\pic {daempfer={0,\mb,\md,$d_2$}};
			\end{scope}
			
			\begin{scope}[xshift=5*\mb cm]
				\pic at (\mb,0){wagen={\mh,\mb,\mr,$m_n$}};
				\pic {feder={0,\mb,\mf,$c_{n}$}};
				\pic {daempfer={0,\mb,\md,$d_{n}$}};
			\end{scope}
			
			\begin{scope}[xshift=7*\mb cm]
				\pic {feder={0,\mb,\mf,$c_{n+1}$}};
				\pic {daempfer={0,\mb,\md,$d_{n+1}$}};
			\end{scope}
			
			\begin{scope}[xshift=8*\mb cm]
				% Wand
				\draw[](0,0) --++ (0,\mh+2*\mr);
				\fill[pattern={Lines[angle=-45, distance={1pt}]},pattern color=black](0,0) rectangle (3pt,\mh+2*\mr);
			\end{scope}
			
		}
	}
}
